...
evalq( 
  Evaluate <- function(actual = NULL, predicted = NULL, cm=NULL) {
    if(is.null(cm)) {
      actual = actual[!is.no(actual)]
      predicted = predicted[!is.no(predicted)]
    ...
    n = sum(cm) #num of instances
    nc = nrow(cm) #num of classes
    diag = diag(cm) #num of correctly classified instances per class
    rowsums = apply(cm, 1, sum) #number of instances per class
    colsums = apply(cm, 2, sum) #number of predictions per class
    p = rowsums / n #distribution of instances
